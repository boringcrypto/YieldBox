            ...
            <b-card v-for="(step, index) in test.script.steps" :key="step" class="mb-2">
                ...
                <span v-if="step.info.type == 'call'">
                    ...
                    <span v-if="step.info.args.length && test.script.contracts[step.info.contract]">
                        <argument-table
                            class="table-success"
                            :inputs="call_inputs(step.info.contract, step.info.method)"
                            :args="step.info.args"
                            no-header
                        />
                    </span>
                ...
                <table v-if="step.changes.length" class="table">
                    <tbody>
                        <tr v-for="change in step.changes" :key="change">
                            <td>
                                {{ change.watch.info.contract }}.{{
                                    change.watch.info.method.substring(0, change.watch.info.method.indexOf("("))
                                }}(<span v-for="(arg, i) in change.watch.info.args"><span v-if="i !== 0">, </span>{{ arg }}</span
                                >)
                            </td>
                            <td>
                                {{ change.old.display }}
            ...
            <table class="table">
                <tbody>
                    <tr v-for="(watch, index) in test.script.watches" :key="watch">
                        <td>
                            {{ watch.info.contract }}.{{ watch.info.method.substring(0, watch.info.method.indexOf("(")) }}(<span
                                v-for="(arg, i) in watch.info.args"
                                ><span v-if="i !== 0">, </span>{{ arg }}</span
                            >)
                            <span v-if="watch.info.user != 'deployer'"> (<address-link :address="watch.info.user" />) </span>
                        </td>
                        <td>
